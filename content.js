...
(function (root, factory) {
    ...
    function normalizeTimeUnit(unit) {
        var normalized = normalizeWhitespace(unit).toLowerCase();
        ...
        return Object.prototype.hasOwnProperty.call(UNIT_SECONDS, normalized) ? normalized : null;
    }
    ...
    function parseCompactNumber(text) {
        var normalized = normalizeWhitespace(text).replace(/,/g, "").replace(/\s+/g, "");
        if (!normalized) {
            return null;
        }

        var match = normalized.match(/^(\d+(?:\.\d+)?)([KMB])?$/i);
        if (!match) {
            return null;
        }
    ...
    function parseViewText(text) {
        ...
        var match = normalized.match(/\b([\d,.]+(?:\.\d+)?\s*[KMB]?)\s+(views?|watching(?:\s+now)?)\b/i);
        if (!match) {
            return null;
        }
    ...
    function parseAgeText(text) {
        ...
        var match = normalized.match(/(?:streamed\s+)?(\d+)\s+(second|minute|hour|day|week|month|year)s?\s+ago/i);
        if (!match) {
            return null;
        }

        var value = Number(match[1]);
        var unit = normalizeTimeUnit(match[2]);
        if (!unit) {
            return null;
        }
    ...
    function parseDurationText(text) {
        ...
        var match = normalized.match(/\b(\d{1,2}:\d{2}(?::\d{2})?)\b/);
        if (match) {
            return buildDurationFromParts(match[1].split(":").map(function (part) {
                return Number(part);
            }));
        }

        var hoursMatch = normalized.match(/(\d+)\s+hours?/i);
        var minutesMatch = normalized.match(/(\d+)\s+minutes?/i);
        var secondsMatch = normalized.match(/(\d+)\s+seconds?/i);
    ...
    function buildDurationFromParts(parts) {
        if (parts.some(function (part) { return Number.isNaN(part); })) {
            return null;
        }
    ...
    function normalizePrefs(rawPrefs) {
        var prefs = rawPrefs || {};
        var minDuration = parseDurationText(prefs.minDurationPref);
        var maxDuration = parseDurationText(prefs.maxDurationPref);
    ...
    function isArtistLabel(label) {
        return /official artist channel|\bartist\b/i.test(normalizeWhitespace(label));
    }
    ...
    function detectLiveFlag(labels, metadataTexts, hasCardContext, hasDuration) {
        ...
        if (texts.some(function (text) {
            return /^live(?:\s+now)?$/i.test(text) || /\b\d[\d,.]*\s*[KMB]?\s+watching(?:\s+now)?\b/i.test(text);
        })) {
            return true;
        }
    ...
    function hasCreatorFilter(prefs) {
        return !(prefs.allowRegularCreator && prefs.allowVerifiedCreator && prefs.allowArtistCreator);
    }

    function hasDurationFilter(prefs) {
        return prefs.minDurationSeconds !== null || prefs.maxDurationSeconds !== null;
    }
    ...
    function evaluateVideo(videoMeta, prefs) {
        var meta = videoMeta || {};
        var normalizedTitle = normalizeSearchText(meta.title || "");
        var reasons = [];
        ...
        if (hasAgeFilter(prefs)) {
            if (!meta.age || typeof meta.age.totalSeconds !== "number") {
                reasons.push("age");
            } else if (meta.age.totalSeconds > (prefs.maxAgeValue * UNIT_SECONDS[prefs.maxAgeUnit])) {
                reasons.push("age");
            }
        }
        ...
        if (hasDurationFilter(prefs)) {
            if (typeof meta.durationSeconds !== "number") {
                ...
                if (prefs.minDurationSeconds !== null && meta.durationSeconds < prefs.minDurationSeconds) {
                    reasons.push("duration");
                }
                if (prefs.maxDurationSeconds !== null && meta.durationSeconds > prefs.maxDurationSeconds) {
                    reasons.push("duration");
                }
            }
        }
        ...
        if (hasKeywordFilter(prefs)) {
            ...
            } else if (!prefs.keywords.some(function (keyword) { return titleMatchesKeyword(normalizedTitle, keyword); })) {
                reasons.push("keywords");
            }
        }
...
(function () {
    ...
    var FilterCore = globalThis.YouTubeFilterCore;
    if (!FilterCore || typeof chrome === "undefined" || !chrome.runtime || typeof document === "undefined") {
        return;
    }
    ...
    var VIDEO_RENDERER_SELECTOR = "ytd-rich-item-renderer, ytd-video-renderer, ytd-grid-video-renderer";
    ...
    var METADATA_ATTRIBUTE_SELECTORS = [
        "a#thumbnail",
        "a#video-title-link"
    ];
    var DURATION_TEXT_SELECTORS = [
        "#time-status #text",
        "#time-status span",
    ...
    var DURATION_ATTRIBUTE_SELECTORS = [
    ...
    var CREATOR_METADATA_ROOT_SELECTORS = [
        "yt-content-metadata-view-model",
        "yt-lockup-metadata-view-model",
        "ytd-channel-name"
    ];
    var CREATOR_ICON_SELECTORS = [
    ...
    function scheduleObserverRefresh(delay) {
        ...
        refreshTimer = setTimeout(refreshObserver, typeof delay === "number" ? delay : 300);
    }
    ...
    function findFeedContainer() {
        for (var i = 0; i < FEED_CONTAINER_SELECTORS.length; i += 1) {
            var container = document.querySelector(FEED_CONTAINER_SELECTORS[i]);
            if (container) {
                return container;
            }
        }
    ...
    function refreshObserver() {
        var nextRoot = findFeedContainer();
    ...
    function handleMutations(mutationList) {
        var cards = collectCardsFromMutations(mutationList);
        if (cards.length === 0) {
            return;
        }
    ...
    function applyFilters() {
        var root = state.observedRoot || findFeedContainer();
        if (!root) {
            return;
        }
    ...
    function applyFilterToCard(card) {
        var decision = FilterCore.evaluateVideo(extractVideoMeta(card), state.prefs);
    ...
    function extractVideoMeta(card) {
        ...
        var metadataTexts = mergeUniqueTexts(
            collectCandidateTexts(card, METADATA_SELECTORS),
            collectAttributeTexts(card, METADATA_ATTRIBUTE_SELECTORS, ["aria-label", "title"])
        );
        var duration = extractDuration(card);
        var featureLabels = collectCandidateTexts(card, FEATURE_BADGE_SELECTORS);
        var creatorBadgeLabels = collectCandidateTexts(card, CREATOR_BADGE_SELECTORS);
        var creatorLabels = extractCreatorLabels(card, creatorBadgeLabels);
        var hasCreatorContext = Boolean(card.querySelector(CREATOR_CONTEXT_SELECTOR));
        var hasCardContext = Boolean(title || metadataTexts.length > 0 || featureLabels.length > 0 || creatorLabels.length > 0 || duration);
        ...
        return {
            ...
            title: title,
            creatorType: FilterCore.classifyCreatorType(creatorLabels, hasCreatorContext),
            isLive: FilterCore.detectLiveFlag(featureLabels, metadataTexts, hasCardContext, Boolean(duration)),
            isSponsored: FilterCore.detectSponsoredFlag(featureLabels, metadataTexts, hasCardContext)
        };
    }

    // Read duration from visible overlay text or accessibility labels.
    function extractDuration(card) {
        var durationTexts = mergeUniqueTexts(
            collectCandidateTexts(card, DURATION_TEXT_SELECTORS),
            collectAttributeTexts(card, DURATION_ATTRIBUTE_SELECTORS, ["aria-label", "title"])
        );
    ...
    function extractCreatorLabels(card, badgeLabels) {
        var creatorLabels = Array.isArray(badgeLabels) ? badgeLabels.slice() : [];
        var creatorRoots = getCreatorRoots(card);
        var creatorHtml = creatorRoots.map(function (root) {
            return root.innerHTML || "";
        }).join(" ");
        var hasCreatorIcon = creatorRoots.some(function (root) {
            return root.querySelector(CREATOR_ICON_SELECTORS.join(", "));
        });
        var hasOfficialArtistMarkup =
            /official_artist_badge|official artist channel|badge_style_type_verified_artist/i.test(creatorHtml) ||
            creatorHtml.indexOf("M9.03 2.242") !== -1 ||
            creatorHtml.indexOf("L14.001 9.7v4.55a2.75") !== -1;
        var hasVerifiedMarkup = /check_circle_thick|badge_style_type_verified|creator_verified/i.test(creatorHtml);
        ...
        if (hasCreatorIcon && !creatorLabels.some(function (label) {
    ...
    function getTitle(card) {
        ...
        for (var i = 0; i < TITLE_SELECTORS.length; i += 1) {
            var element = card.querySelector(TITLE_SELECTORS[i]);
            if (!element) {
                continue;
            }
    ...
    function pushTitleCandidate(candidates, text) {
        var normalized = sanitizeTitleCandidate(text);
    ...
    function sanitizeTitleCandidate(text) {
        var normalized = FilterCore.normalizeWhitespace(text);
        if (!normalized) {
            return null;
        }

        if (/\bby\b/i.test(normalized) && (/\bviews?\b/i.test(normalized) || /\bago\b/i.test(normalized))) {
    ...
    function pushUniqueValue(values, value, seen) {
        var nextSeen = seen || new Set(values);
